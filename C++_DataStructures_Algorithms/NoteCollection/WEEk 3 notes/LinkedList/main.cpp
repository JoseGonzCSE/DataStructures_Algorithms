...
void test() {
	...
	string data[] = { "Hello","What", "is","up" };
	cout << "Hey wow look at that list using our Profs method!" << endl;

	for (int i = 0; i < 4; i++)
	{
		if (list2Ptr->insert(i + 1, data[i]))
			cout << "Inserted " << list2Ptr->getEntry(i + 1)
	...
	cout << "Now that you know the list, lets reverse it!" << endl;
	list2Ptr->reverse();
