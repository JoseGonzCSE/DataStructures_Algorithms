...
bool LinkedList<ItemType>::insert(int newPosition, const ItemType& newEntry)
{
   ...
   if (ableToInsert)
   {
      ...
      if (getLength()==0) {
          ...
          newNodePtr->setPrev(prevPtr->getPrev());
          prevPtr->setNext(newNodePtr);
      }
...
//swap each prev and next
template<class ItemType>
void LinkedList<ItemType>::reverse() {

    Node<ItemType>* currentPtr = getNodeAt(1);
    Node<ItemType>* tempPtr = currentPtr->getNext();
    for (int i = 0; i < itemCount; i++) {
        currentPtr->setNext(currentPtr->getPrev());
        currentPtr->setPrev(tempPtr);
        currentPtr = tempPtr;
